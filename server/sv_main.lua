...
Citizen.CreateThread(function() 
    ...
    EventsModule.RegisterServer('quantum-financials/server/monitor-account', function(Source, AccountData)
    ...
    CallbackModule.CreateCallback('quantum-financials/server/get-accounts', function(Source, Cb)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local AccountTable = {}
                ...
                DatabaseModule.Execute("SELECT * FROM player_accounts", {}, function(AccountsData) 
                    if AccountsData[1] ~= nil then
                        for k, v in pairs(AccountsData) do
                            if exports['quantum-business']:IsPlayerInBusiness(Player, v.Name) and exports['quantum-business']:HasBusinessPermission(Player, v.Name, 'account_access') then
                                local AccountData = {}
                                AccountData['AccountOwner'] = exports['quantum-business']:GetBusinessOwnerName(v.Name)
    ...
    CallbackModule.CreateCallback('quantum-financials/server/get-current-balance', function(Source, Cb, AccountId)
        local Player = PlayerModule.GetPlayerBySource(Source)
        Cb(GetCurrentAccountBalance(AccountId, Player))
    end)

    CallbackModule.CreateCallback('quantum-financials/server/withdraw-money', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    CallbackModule.CreateCallback('quantum-financials/server/deposit-money', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local CurrentBalance = GetCurrentAccountBalance(Data['AccountId'])
    ...
    CallbackModule.CreateCallback('quantum-financials/server/transfer-money', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
...
RegisterNetEvent('quantum-financials/server/sync-main-bank', function(Source, BankAmount)
    local Player = PlayerModule.GetPlayerBySource(Source)
    if not Player then 
